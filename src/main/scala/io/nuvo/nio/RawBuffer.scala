...
object RawBuffer {
  ...
  def wrap(bs: Array[Byte]) = new RawBuffer(java.nio.ByteBuffer.wrap(bs))
}
...
class RawBuffer(val buffer: java.nio.ByteBuffer) extends Ordered[RawBuffer] {
  ...
  private val thisClass: Class[_ <: RawBuffer] = this.getClass()
  ...
  def getBoolean(): Boolean = {
    val b = this.get()
    if (b == RawBuffer.FALSE) false else true
  }
  ...
  def << (b: Byte): RawBuffer = put(b)

  def putByte(b: Byte): RawBuffer = put(b)

  def putBoolean(b: Boolean): RawBuffer = if (b) this.put(RawBuffer.TRUE) else this.put(RawBuffer.FALSE)

  def << (b: Boolean): RawBuffer = putBoolean(b)
  ...
  def << (v: Char): RawBuffer = putChar(v)
  ...
  def << (s: Short): RawBuffer = putShort(s)
  ...
  def << (l: Long): RawBuffer = putLong(l)
  ...
  def << (v: Float): RawBuffer = putFloat(v)
  ...
  def << (v: Double): RawBuffer = putDouble(v)
  ...
  def << (v: String): RawBuffer = putString(v)
  ...
  def putObject[T <: AnyRef](o: T, sf: SerializationFormat)(implicit sr: SerializerRegistry): RawBuffer = sf match {
    case NuvoSF => {
      ...
      val (oserializers, kserializers) = sr.resolve(typeName).getOrElse (
      {
        sr.registerType(typeName)
        sr.lookup(typeName).get
      })

      oserializers._1.map(_.invoke(null, this, o, sr))
      this
    }
    case JavaSF => {
  ...
  def getObject[T <: AnyRef](sf: SerializationFormat)(implicit sr: SerializerRegistry): T = sf match {
    case NuvoSF => {
      ...
      val result = oserializer._2.map(_.invoke(null, this, sr).asInstanceOf[T]).get
